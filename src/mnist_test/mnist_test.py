...
def pertubation_test(label, pertubation):
    ...
    # x_ptb = x[y == (label+1)][0]
    logits = cnn_profiler.get_mid([None, 28, 28, 1], [None, 10], x_ptb, anchor=-1, filter=None)
    print(logits)
    plt.subplot(211)
    plt.imshow( x_ori)
    plt.subplot(212)
    plt.imshow( x_ptb)
    plt.show()
    test_result = cnn_profiler.test([None, 28, 28, 1], [None, 10], x_ptb)
